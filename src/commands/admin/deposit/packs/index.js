...
export default async (sender, msg) => {
  try {
    ...
    const amountOfPacks = parseInt(input.replace('!DEPOSITPACKS ', ''), 10);
    ...
    log.adminChat(sender.getSteamID64(), `[ !DEPOSITPACKS ${amountOfPacks} ]`);
    ...
    if (error.message.includes('Insufficient number of booster Pack(s)')) {
      chatMessage(sender, messages.error.outofstock.packs.them);
    } else if (
